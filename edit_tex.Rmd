...
library(stringr)
```
This .Rmd post-processes paper.tex file to fix bugs and formatting issues.

I run code chunk by chunk rather as needed rather than knitting rmd 

I then compile paper.tex manually
...
# Fix float placement bug caused by kableExtra
# When using kable_classic_2(latex_options = "HOLD_position"),
# the [H] float  is inserted as a separate line in the table environment
# instead adding it after \begin{table} 
# This function searches for misplaced [H] lines and moves them
# to the correct position as an optional argument to \begin{table}[H],
fix_table_placement_in_tex <- function(tex_path) {
  lines <- readLines(tex_path)
  new_lines <- lines
  i <- 1
  
  while (i < length(new_lines)) {
    if (grepl("^\\\\begin\\{table\\}\\s*$", new_lines[i])) {
      # Search ahead for a standalone [H] line
      for (j in seq(i + 1, min(i + 5, length(new_lines)))) {
        if (grepl("^\\s*\\[H\\]\\s*$", new_lines[j])) {
          # Insert [H] into \begin{table}
          new_lines[i] <- "\\begin{table}[H]"
          new_lines <- new_lines[-j]
          message("✔ Fixed misplaced [H] at line ", j)
          break
        }
      }
    }
  ...
  writeLines(new_lines, tex_path)
  message("✔ Updated .tex file written to: ", tex_path)
}

fix_table_placement_in_tex("paper.tex")
```
