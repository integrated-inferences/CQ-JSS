...
keywords-formatted: [Causal Models, Stan, Bayesian Updating]
author:
  - name: Till Tietz
    affiliations:
      - name: WZB
...
+---------------+--------------------------+---------------------+-------------------------+------------------------------------------------------------------------+
| `autobounds`  | @duarte_automated_2023   | [Python]{.proglang} | not readily installable | -   bounding causal effects                                            |
...
A model is defined in one step in `CausalQueries` using a `dagitty` syntax [@textor_robust_2016] in which the structure of the model is provided as a statement.
...
The primary output from `update_model()` is a model with an attached posterior distribution over model parameters, stored as a data frame in `model$posterior_distribution`. In addition, a distribution of causal types is stored by default and the `stanfit` object and a distribution over event probabilities are optionally saved.
...
with row names indicating nodal types and columns realized values. Intervening on $X$ [see @pearl_causality_2009] with $do(X=1)$ yields:
...
This can be done for a list of queries using `query_distribution()` function:
...
We present a brief summary of model updating benchmarks. First benchmark considers the effect of model complexity on updating time. Second benchmark considers the effect of data size on model complexity. We run $4$ parallel chains for each model. Results of the benchmarks are presented in @tbl-bench1 and @tbl-bench2 respectively.
...
::: callout
The approach to generating a generic function to create Stan code that can take data from arbitrary models was developed in key contributions by [Jasper Cooper](http://jasper-cooper.com/) and [Georgiy Syunyaev](http://gsyunyaev.com/). [Lily Medina](https://lilymedina.github.io/) did magical work pulling it all together and developing approaches to characterizing confounding and defining estimands. [Clara Bicalho](https://clarabicalho.github.io/) helped figure out the syntax for causal statements. [Julio S. Solís Arce](https://www.gov.harvard.edu/directory/julio-s-solis-arce/)made many key contributions figuring out how to simplify the specification of priors. [Merlin Heidemanns](https://merlinheidemanns.github.io/website/) figured out the `rstantools` integration and made myriad code improvements. [Till Tietz](https://github.com/till-tietz) revamped the entire package and improved every part of it.
:::
...
[^5]: For a discussion of implementation of this approach in Stan see discussion (here)\[https://discourse.mc-stan.org/t/ragged-array-of-simplexes/1382\].
