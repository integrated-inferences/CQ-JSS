...
The parameters data frame keeps track of parameter values and priors for parameters but does not provide a mapping between parameters and the probability of causal types. The parameter matrix provides this mapping. The parameters matrix has a row for each parameter and a column for each causal type. For instance:
...
The probability of a causal type is given by the product of the parameter values for parameters whose row in the parameters matrix contains a $1$. Later (e.g., in @sec-confounding), we will see examples where the parameters matrix helps track parameters created when confounding is added to a model.
...
The function `update_model()` can be used to update a model as follows:
