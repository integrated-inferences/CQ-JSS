...
Optionally, users can choose whether to keep and append additional elements to `stan_objects.` For instance, they can specify whether to include `w`, which maps parameters to event probabilities, as well as the `stanfit` object, the output generated by stan
...
=======
We can map from causal types to data types by propagating realized values on nodes forward in the DAG, moving from exogenous or intervened upon nodes to their descendants in generational order. The `realise_outcomes` functions achieves this by traversing the DAG, while recording for each node's nodal types, the values implied by realizations on the node's parents or by controlled values of a node's parents. 

By way of example, consider the first causal type of the $X \rightarrow Y$ model above:
>>>>>>> main
