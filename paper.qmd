...
For concreteness: table illustrating all these values fo $X\rightarrow Y$ model
...
We illustrate by implementing a replication from @ii2023 of the analysis in @chickering1996clinician using `CausalQueries`. @chickering1996clinician seek to draw inference in the presence of imperfect compliance. Their data on $Z$, $X$, and $Y$ is complete for all units and looks, in compact form, as follows:
...
Using `CausalQueries` we can define the model, update it with data, and query it. 

We make three queries: first for the average causal effect of $X$ on $Y$. Second for the effect for units for which $X=0$ and $Y=0$. Last we also assess the average treatment effect *for* "compliers": units for which $X$ responds positively to $Y$.
