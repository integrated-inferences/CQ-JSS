...
in words: we restrict by removing types for which $X$ is decreasing in $Z$.
...
Here and in general, setting restrictions typically involves using causal syntax; see Section @sec-syntax for a guide the syntax used by `CausalQueries`.
...
#| echo: false
#| eval: true
make_model("X -> Y") |> get_parmap() |>   knitr::kable(
    digits = 2,
    booktabs = TRUE,
    align = "c",
    escape = TRUE, 
