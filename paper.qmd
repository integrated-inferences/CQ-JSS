...
Once defined, a model can be graphed by calling the `plot()` method defined for the objects with class `causal_model` produced by  `make_model()` function. Options can be provided to `plot()` are passed to `CausalQueries:::plot_dag()`.

@fig-plots shows figures generating by plotting `lipids_model` with and without options. The  plots  have class `c("gg", "ggplot")` and so will accept any additional modifications available via the `ggplot2` package.
...
The resulting parameter matrix is shown in @tbl-confound-param-matrix. Importantly, the $P$ matrix works as before, despite confounding. We can assess the probability of causal types by multiplying the probabilities of the constituent parameters.
...
data.frame(Model = statements |> 
             lapply(function(s) paste0("`", s, "`")) |> unlist(),
...
#| eval: true
#| echo: true
# snapshot of the posterior distribution
get_posterior_distribution(lipids_model)[1:5, 1:5]

```

In addition, a distribution of causal types is stored by default and the `stanfit` object and a distribution over event probabilities are optionally saved. 



```{r}
lipids_model <- lipids_model |> update_model(keep_fit = TRUE)

lipids_model$stan_objects$stan_fit |> print()
```
