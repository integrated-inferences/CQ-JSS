...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, interior hidden, enhanced, frame hidden, sharp corners, breakable, boxrule=0pt]}{\end{tcolorbox}}\fi
\makeatother
\ifLuaTeX
...
{\centering \includegraphics{paper_files/figure-pdf/queryplot-1.pdf}
...
\textbf{?@tbl-core} summarizes features of a causal model that can be
examined using \texttt{inspect()}.

\input{tables/core_elements.tex}
...
\input{tables/nodal_types.tex}

\textbf{?@tbl-joint} has eight interior elements so that an
...
#> 
#> Samples were drawn using NUTS(diag_e) at Mon Jul  7 18:56:17 2025.
...
#> 
#> Samples were drawn using NUTS(diag_e) at Mon Jul  7 18:56:20 2025.
#> For each parameter, n_eff is a crude measure of effective sample size,
#> and Rhat is the potential scale reduction factor on split chains (at 
#> convergence, Rhat=1).
\end{verbatim}
...
\includegraphics{paper_files/figure-pdf/case-level-query-1.pdf}
...
\includegraphics{paper_files/figure-pdf/unnamed-chunk-52-1.pdf}
...
\includegraphics{paper_files/figure-pdf/fig-batch-1.pdf}
...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, left=2mm, bottomrule=.15mm, colback=white, leftrule=.75mm, toprule=.15mm, opacityback=0, colframe=quarto-callout-color-frame, breakable, arc=.35mm]
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.5618}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.2360}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.2022}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\(X1 \rightarrow Y\) & 6 & 1.18 \\
\(X1 \rightarrow Y; X2 \rightarrow Y\) & 20 & 2.52 \\
\(X1\rightarrow Y;X2\rightarrow Y;X3\rightarrow Y\) & 262 & 48.99 \\
...
\bottomrule\noalign{}
\endlastfoot
\(X1 \rightarrow Y\) & 10 & 863.05 \\
\(X1 \rightarrow Y\) & 100 & 1201.23 \\
\(X1 \rightarrow Y\) & 1000 & 2258.39 \\
\(X1 \rightarrow Y\) & 10000 & 6054.22 \\
