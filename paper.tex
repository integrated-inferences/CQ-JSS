...
which \(X=0\) and \(Y=0\). The last is the average treatment effect for
``compliers'': units for which \(X\) responds positively to \(Z\). Thus
two of these queries are conditional queries, with one conditional on a
...
\texttt{dagitty} syntax \citep{dagitty} in which the structure of the
model is provided as a statement.
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.1263}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.8737}}@{}}
...
types), as shown in \textbf{?@tbl-joint}.

\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.2460}}
...
\textbf{?@tbl-joint} has eight interior elements and so an unconstrained
joint distribution would have \(7\) degrees of freedom. A no confounding
...
Table Table~\ref{tbl-batch-query} shows output from a single call to
\texttt{query\_model}.
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, breakable, left=2mm, leftrule=.75mm, rightrule=.15mm, arc=.35mm, bottomrule=.15mm, toprule=.15mm, colback=white]
