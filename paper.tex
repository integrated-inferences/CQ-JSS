...
For concreteness: table illustrating all these values fo
...
We illustrate by implementing a replication from \citet{ii2023} of the
analysis in \citet{chickering1996clinician} using
\texttt{CausalQueries}. \citet{chickering1996clinician} seek to draw
inference in the presence of imperfect compliance. Their data on \(Z\),
\(X\), and \(Y\) is complete for all units and looks, in compact form,
as follows:
...
data <- data.frame(
  ...
  strategy = "ZXY",
  count = c(158, 52, 0, 23, 14, 12, 0, 78)
)
\end{verbatim}
...
We make three queries: first for the average causal effect of \(X\) on
\(Y\). Second for the effect for units for which \(X=0\) and \(Y=0\).
Last we also assess the average treatment effect \emph{for}
``compliers'': units for which \(X\) responds positively to \(Y\).
...
Chain 1:                0.898 seconds (Sampling)
...
Chain 2: 
Chain 2:  Elapsed Time: 0.991 seconds (Warm-up)
Chain 2:                0.79 seconds (Sampling)
Chain 2:                1.781 seconds (Total)
Chain 2: 
...
Chain 3:                0.802 seconds (Sampling)
Chain 3:                1.67 seconds (Total)
Chain 3: 
...
Chain 4: 
Chain 4:  Elapsed Time: 0.927 seconds (Warm-up)
Chain 4:                0.881 seconds (Sampling)
Chain 4:                1.808 seconds (Total)
Chain 4: 
\end{verbatim}

\begin{verbatim}
results |>
  select(query, given, mean, sd) |>
  kable(digits = 2, caption = "Rows 1 and 2 replicate results in Chickering and Pearl (1997); row 3 returns inferences for complier average effects.")
\end{verbatim}
...
\begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, rightrule=.15mm, colback=white, leftrule=.75mm, arc=.35mm, bottomrule=.15mm, left=2mm, breakable, opacityback=0]
