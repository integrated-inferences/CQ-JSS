...
facilitate cannonicalization. The essential difference between the
methods; however, lies in their approach to evaluating queries.

Both \texttt{autobounds} and \texttt{causaloptim} build on seminal
approaches in \citet{balke1997bounds} to construct bounds of queries,
...
a posterior over the causal model which is then queried (consistent with
\citet{chickering1996clinician} and \citet{zhang2022pci}). A key
...
satisfies the conditions in \citet{angrist1996identification}, then
...
of \(E\) has 65544 parameters. Moving from binary to non binary nodes
has similar effects. The restriction to binary nodes is for
...
used for these purposes (see section 9.4 of \citet{ii2023}). The targets
of inference are usually case level quantities or population quantities
and \texttt{CausalQueries} is not well suited for estimating sample
quantities.
...
ways that a node responds as ``nodal type.'' The set of nodal types
corresponds to principal strata familiar, for instance, in the study of
instrumental variables \citep{frangakis2002principal}.
...
updated parameters generated by \texttt{stan}. \\
...
\texttt{stan\_fit} & The \texttt{stanfit} object generated by
stan,optionally appended to \texttt{stan\_objects.} \\
\end{longtable}
...
prints the parameters data frame (see Table~\ref{tbl-lipidspar} for an
example). Each row in the data frame corresponds to a single parameter.
...
Below we will see examples where the parameters data frame helps keep
...
\caption{\label{tbl-confound-params-mat}\textbf{?(caption)}}\begin{minipage}[t]{\linewidth}
\subcaption{\label{tbl-confound-params-mat-1}}
...
This table has eight interior elements and so an unconstrained joint
...
By default priors are set to unit vector, corresponding to uniform
priors. To retrieve the model's priors we can run the following code:
...
grouped by data ``strategy''---an indicator of the nodes for which data
was gathered. \texttt{CausalQueries} includes functions that lets you
move between these two forms in case of need.
...
\caption{\label{fig-parmap}\textbf{?(caption)}}

\end{figure}

For example in table Figure~\ref{fig-parmap} the probability of data
type \texttt{X0Y0}, \(w_{00}\) is
...
Intervening on \(X\) (see \citet{pearl2009causality}) with \(do(X=1)\)
yields:
...
The \texttt{query\_model()} function takes list of models, causal
queries, and conditions (\texttt{given}), calculates estimands given
prior or posterior distributions and reports summaries of these
distributions. The result is a data frame which can be displayed as a
table or used for graphing.
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, left=2mm, rightrule=.15mm, toprule=.15mm, arc=.35mm, breakable, colback=white, bottomrule=.15mm, opacityback=0]
...
Updating is implemented in \texttt{stan} using a single flexible
\texttt{stan} model.

The data provided to \texttt{stan} is generated by the internal function
\texttt{prep\_stan\_data} which returns a list of objects that
...
One can examine the data passed to stan given a model and compact data
thus:

\begin{verbatim}
R> CausalQueries:::prep_stan_data(model, compact_data)
\end{verbatim}

The code for the \texttt{stan} model is show below. After defining a
helper function the code starts with a block declaring what input data
is to be expected. Then there is a characterization of parameters and
the transformed parameters. Then the likelihoods and priors are
provided. At the end there is a block for generated quantities which can
be used to append a posterior distribution of causal types to the model.
