...
\texttt{do} calculus. The parameters of focus are conditional
probability distribution tables rather than principal strata (causal
types). This approach limits the types of queries that can be posed and
the kinds of expert information that can be incorporated. For instance
knowledge of conditional probability distributions are not sufficient to
make claims about (or provide priors with respect to) effect
monotonicity, complier effects, or the probability of causation. However
it makes it possible to address simple queries efficiently with much
larger models.
...
\texttt{CausalQueries} in the queries it allows.
...
defined implicitly via main nodes and edges in \texttt{CausalQueries} vs
...
employ \texttt{CausalQueries} to query models with non-binary nodes but,
in general, the computational complexity makes analysis of these models
prohibitively costly.\footnote{For more on computation constraints and
  strategies to update and query large models see the associated package
  \texttt{CausalQueriesTools} available via
...
The first element is a \texttt{statement} which defines the relations of
causal ancestry between nodes. The second element, \texttt{dag}, is a
data frame that characterizes the parent-child relationships within the
model. The element \texttt{nodes} is simply a list of the names of the
nodes in the model. Fourth, the \texttt{parents\_df,} is a table listing
the nodes, indicating if they are ``root'' nodes (nodes with no parents
among the set of specified nodes) and showing how many parents each node
has.
...
\item
  ...
  parameter.
\item
  \texttt{given} indicates nodal types that are conditioned upon for
  cases in which parameters are needed to characterize joint
  distributions of nodal types.
\item
...
\item
  ...
  (hyperparameters) for parameters in a set. Values of \(1\) (\(.5\))
  for all parameters in a set imply uniform (Jeffreys) priors over this
  set.
\end{itemize}
...
These interpretations are automatically provided when the nodal types
are accessed via \texttt{grab(model,\ "nodal\_types")}.
...
object when a model is created. Users can access causal types using the
...
probability of causal types. The parameter matrix provides this mapping.
The parameter matrix has a row for each parameter and a column for each
...
parameter values for parameters whose row in the parameter matrix
contains a \(1\). Later (e.g., in Section~\ref{sec-confounding}), we
will see examples where the parameter matrix helps track parameters
created when confounding is added to a model.

The parameter matrix is generated on the fly as needed, but to speed up
some repeated operations it can be useful to add it to the model
manually. This can be done using the \texttt{set\_parameter\_matrix()}
function as follows:
...
When a \texttt{dagitty} statement is provided to \texttt{make\_model()},
...
\protect\hyperlink{sec-parallel}{Appendix A} how users can utilize
...
#> |model      |query           |  mean|    sd|
#> |:----------|:---------------|-----:|-----:|
#> |uncensored |Y[X=1] - Y[X=0] | 0.594| 0.195|
#> |censored   |Y[X=1] - Y[X=0] | 0.010| 0.323|
\end{verbatim}
...
#> X.0         0.50    0.01 0.29   0.02  0.24  0.50  0.75  0.97  3129    1
#> X.1         0.50    0.01 0.29   0.03  0.25  0.50  0.76  0.98  3129    1
#> Y.00        0.25    0.00 0.20   0.01  0.09  0.21  0.39  0.72  2065    1
#> Y.10        0.25    0.00 0.19   0.01  0.09  0.21  0.37  0.70  4293    1
#> Y.01        0.25    0.00 0.19   0.01  0.09  0.20  0.36  0.71  4665    1
#> Y.11        0.25    0.00 0.20   0.01  0.09  0.21  0.37  0.72  4480    1
#> lp__       -7.54    0.05 1.71 -11.96 -8.33 -7.16 -6.32 -5.43  1220    1
#> 
#> Samples were drawn using NUTS(diag_e) at Fri May 31 16:47:42 2024.
...
#> 
#>             mean se_mean   sd   2.5%   25%   50%   75% 97.5% n_eff Rhat
#> lambdas[1]  0.50    0.01 0.28   0.03  0.25  0.50  0.74  0.97  3084    1
#> lambdas[2]  0.50    0.01 0.28   0.03  0.26  0.50  0.75  0.97  3084    1
#> lambdas[3]  0.25    0.00 0.20   0.01  0.09  0.20  0.36  0.72  1871    1
#> lambdas[4]  0.25    0.00 0.19   0.01  0.09  0.21  0.37  0.70  4027    1
#> lambdas[5]  0.25    0.00 0.19   0.01  0.10  0.21  0.37  0.70  4803    1
#> lambdas[6]  0.25    0.00 0.19   0.01  0.09  0.20  0.37  0.71  4622    1
#> types[1]    0.12    0.00 0.13   0.00  0.03  0.08  0.18  0.48  2198    1
#> types[2]    0.13    0.00 0.13   0.00  0.03  0.08  0.18  0.49  1996    1
#> types[3]    0.13    0.00 0.13   0.00  0.03  0.08  0.18  0.49  3277    1
#> types[4]    0.13    0.00 0.13   0.00  0.03  0.08  0.18  0.47  3574    1
#> types[5]    0.12    0.00 0.13   0.00  0.03  0.08  0.18  0.47  3589    1
#> types[6]    0.13    0.00 0.13   0.00  0.03  0.08  0.18  0.50  3657    1
#> types[7]    0.12    0.00 0.13   0.00  0.03  0.08  0.17  0.48  3813    1
#> types[8]    0.13    0.00 0.13   0.00  0.03  0.08  0.18  0.50  3304    1
#> lp__       -7.48    0.04 1.60 -11.46 -8.32 -7.10 -6.30 -5.45  1422    1
#> 
#> Samples were drawn using NUTS(diag_e) at Fri May 31 16:47:48 2024.
#> For each parameter, n_eff is a crude measure of effective sample size,
#> and Rhat is the potential scale reduction factor on split chains (at 
#> convergence, Rhat=1).
\end{verbatim}
...
#> |:---------------|:----------|-----:|-----:|
#> |Y[X=1] > Y[X=0] |TRUE       | 0.666|    NA|
#> |Y[X=1] > Y[X=0] |FALSE      | 0.402| 0.327|
\end{verbatim}
...
A & ATE & - & posteriors & FALSE & 0.56 & 0.10\\
B & ATE & - & posteriors & FALSE & 0.56 & 0.10\\
A & ATE & Y==1 \& X==1 & priors & FALSE & 0.51 & 0.22\\
\addlinespace
B & ATE & Y==1 \& X==1 & priors & FALSE & 0.50 & 0.25\\
A & ATE & Y==1 \& X==1 & posteriors & FALSE & 0.95 & 0.04\\
B & ATE & Y==1 \& X==1 & posteriors & FALSE & 0.95 & 0.04\\
A & POS & - & priors & FALSE & 0.25 & 0.13\\
B & POS & - & priors & FALSE & 0.25 & 0.14\\
\addlinespace
A & POS & - & posteriors & FALSE & 0.61 & 0.10\\
B & POS & - & posteriors & FALSE & 0.61 & 0.10\\
A & POS & Y==1 \& X==1 & priors & FALSE & 0.51 & 0.22\\
B & POS & Y==1 \& X==1 & priors & FALSE & 0.50 & 0.25\\
...
A & ATE & - & posteriors & TRUE & 0.56 & NA\\
B & ATE & - & posteriors & TRUE & 0.56 & NA\\
\addlinespace
A & ATE & Y==1 \& X==1 & priors & TRUE & 0.51 & NA\\
...
A & POS & Y==1 \& X==1 & priors & TRUE & 0.51 & NA\\
...
\begin{tcolorbox}[enhanced jigsaw, breakable, opacityback=0, arc=.35mm, rightrule=.15mm, bottomrule=.15mm, colback=white, left=2mm, toprule=.15mm, leftrule=.75mm]
