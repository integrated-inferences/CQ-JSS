...
We have a \(2\)-dimensional Dirichlet distribution over the \(X\) nodal
types (equivalently, a Beta distribution) and a \(4\)-dimensional
Dirichlet over the \(Y\) nodal types.
...
\caption{\label{fig-query-dist}Prior on ``Probability \(Y\) is
increasing in \(X\)''.}

\end{figure}

Resulting distribution is plotted in Figure~\ref{fig-query-dist}.
...
\begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, opacityback=0, colback=white, arc=.35mm, leftrule=.75mm, left=2mm, breakable, rightrule=.15mm, bottomrule=.15mm]
