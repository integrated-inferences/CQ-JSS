...
causal relations between nodes consistent with the causal statement (2)
determine a set of parameters needed to characterize distributions over
these types (3) update beliefs over distributions of causal types, using
a \texttt{stan} model plus data and (4) pose a wide range of causal
queries of the model, using either the prior distribution, the posterior
...
key quantities that are used for all inferential tasks.
...
This formaula can be read from the parameter mapping matrix by combining
...
of this kind of sampling naturally by asseevent\_probabilitiesssing the
probability of observing a particular pattern of data within each data
strategy. For a discussion see Section 9.2.3.2 of
...
{\centering \includegraphics{paper_files/figure-pdf/unnamed-chunk-41-1.pdf}
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, breakable, colback=white, left=2mm, bottomrule=.15mm, rightrule=.15mm, opacityback=0, arc=.35mm, toprule=.15mm]
