...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[breakable, borderline west={3pt}{0pt}{shadecolor}, frame hidden, sharp corners, boxrule=0pt, interior hidden, enhanced]}{\end{tcolorbox}}\fi
\makeatother
...
\hypersetup{
  pdftitle={Making, Updating, and Querying Causal Models with CausalQueries},
...
\texttt{CausalQueries}}
\Plaintitle{Making, Updating, and Querying Causal Models with
...
the local (complier) average treatment effects (``LATE'').
...
to let users define an arbitrary DAG and pose any queries on it, using a
...
for computational, not conceptual, reasons.\footnote{For more on
...
structure and then place probability distributions over models
...
Elements 1 - 3 of definition 1 define a ``\textbf{structural causal
model}.'' These components define a causal structure, specifying which
endogenous nodes are (possible) direct causes of a node, \(Y_j\), given
the other nodes in a model. These nodes are referred to as the
``parents'' of \(Y_j\), denoted as \(PA_j\) (where uppercase \(PA_j\)
represents the collection of nodes, and lowercase \(pa_j\) represents a
specific set of values these nodes might assume).
...
responses of a node to its parents, which we call ``nodal types'' (often
referred to as ``response functions''). If a node \(i\) can take on
\(k_i\) possible values, then the set of possible values that the
parents of \(j\) can assume is \(m_j :=\prod_{i\in PA_j}k_i\).
Consequently, there are \(k_j^{m_j}\) different ways that node \(j\)
might respond to its parents. For binary nodes, this simplifies to
...
\texttt{CausalQueries} this is done using subscripts that indicate the
...
\texttt{CausalQueries} treats the set of nodal types of \(Y_j\) as the
...
Recall \(\lambda^Y_{01}=\Pr(\theta^Y = \theta^Y_{01})\) and the
subscripts (\(0\), \(00\), \(01\), dots) summarizes the responses of
...
\texttt{CausalQueries} and the key functionalities for making, updating,
...
\texttt{set\_priors}, as describe in sections Section~\ref{sec-make} and
...
defined its structure. \texttt{CausalQueries} offers straightforward
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{paper_files/figure-pdf/fig-plots-1.pdf}
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{paper_files/figure-pdf/fig-plots-2.pdf}
...
that \texttt{grab()} is quiet and does not produce a printout, whereas
\texttt{inspect()} does.
...
#> 
#> Samples were drawn using NUTS(diag_e) at Thu Jul  3 10:55:52 2025.
...
possible problems. These warnings are retained by \texttt{CausalQueries}
...
#> 
#> Samples were drawn using NUTS(diag_e) at Thu Jul  3 10:56:10 2025.
#> For each parameter, n_eff is a crude measure of effective sample size,
#> and Rhat is the potential scale reduction factor on split chains (at 
#> convergence, Rhat=1).
\end{verbatim}
...
expression. For instance \texttt{"Y{[}X=1{]}==1\ :\textbar{}:\ X==0"}
asks for the probability that \(Y=1\) when \(X\) is set to 1 for a case
in which in fact \(X=0\). The entire query then becomes: for what units
does the \texttt{query} condition hold among those units for which the
...
Such values are important for defining queries such as the ``natural
direct effect'' \citep{pearl2022direct}.
...
\pandocbounded{\includegraphics[keepaspectratio]{paper_files/figure-pdf/case-level-query-1.pdf}}
...
this data type, so this expectation has an uncertainty attached to it,
...
\pandocbounded{\includegraphics[keepaspectratio]{paper_files/figure-pdf/unnamed-chunk-28-1.pdf}}
...
\toprule
label & model & query & given & using & case\_level & mean & sd\\
\midrule
ATE & Unrestricted & Y[X=1] - Y[X=0] & - & priors & FALSE & 0.00 & 0.20\\
ATE & Restricted & Y[X=1] - Y[X=0] & - & priors & FALSE & 0.00 & 0.23\\
ATE & Unrestricted & Y[X=1] - Y[X=0] & - & posteriors & FALSE & 0.56 & 0.10\\
...
\addlinespace
ATE & Unrestricted & Y[X=1] - Y[X=0] & - & posteriors & TRUE & 0.56 & NA\\
...
\begin{tcolorbox}[enhanced jigsaw, breakable, rightrule=.15mm, colframe=quarto-callout-color-frame, colback=white, arc=.35mm, left=2mm, opacityback=0, toprule=.15mm, leftrule=.75mm, bottomrule=.15mm]
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2360}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2022}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
\(X1 \rightarrow Y\) & 6 & 7.24 \\
\(X1 \rightarrow Y; X2 \rightarrow Y\) & 20 & 10.04 \\
\(X1\rightarrow Y;X2\rightarrow Y;X3\rightarrow Y\) & 262 & 90.04 \\
...
\bottomrule\noalign{}
\endlastfoot
\(X1 \rightarrow Y\) & 10 & 8.81 \\
\(X1 \rightarrow Y\) & 100 & 8.72 \\
\(X1 \rightarrow Y\) & 1000 & 11.64 \\
\(X1 \rightarrow Y\) & 10000 & 18.73 \\
