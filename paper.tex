...
make, update, and query causal models. Users provide a causal statement
of the form
...
binary variables. \texttt{CausalQueries} can then (1) identify the set
of principal strata---causal types---required to characterize all
possible types of causal relations between nodes consistent with the
causal statement (2) determine a set of parameters needed to
characterize distributions over these types (3) update beliefs over the
distribution of causal types, using a \texttt{stan} model and (4) pose a
...
The function \texttt{query\_model()} can then be used to ask a wide
...
\texttt{get\_parameter\_matrix(confounded)}.
...
1 & ATE & - & posteriors & FALSE & 0.76 & 0.12\\
...
\begin{tcolorbox}[enhanced jigsaw, breakable, colback=white, colframe=quarto-callout-color-frame, left=2mm, arc=.35mm, bottomrule=.15mm, rightrule=.15mm, toprule=.15mm, leftrule=.75mm, opacityback=0]
