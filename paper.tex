...
Queries are posed and answered in \texttt{CausalQueries} by \ldots{}
TEXT HERE
...
an effect in this case (but that there wouldn't be were \(M=1\)).
...
The \texttt{stats} argument let users specify alternative summary
statistics and indicate. The \texttt{expand\_grid} argument lets users
specify whether all combinations of list elements should be generated.
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, opacityback=0, breakable, leftrule=.75mm, toprule=.15mm, bottomrule=.15mm, left=2mm, rightrule=.15mm, arc=.35mm]
