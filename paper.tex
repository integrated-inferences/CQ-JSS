...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[frame hidden, breakable, borderline west={3pt}{0pt}{shadecolor}, sharp corners, boxrule=0pt, enhanced, interior hidden]}{\end{tcolorbox}}\fi
\makeatother
...
\vspace{1em}
\centering
\includegraphics[keepaspectratio,width=0.9\linewidth]{paper_files/figure-pdf/queryplot-1.pdf}}
...
for computational, not conceptual, reasons.\footnote{For more on
  ...
  large models, see the \pkg{CausalQueriesTools} package available via
...
\begin{CodeInput}
R> model_restricted <- lipids_model |> set_restrictions("X[Z = 1] < X[Z = 0]")
\end{CodeInput}
...
\begin{CodeChunk}
\begin{CodeInput}
R> lipids_model |> set_priors(param_names = c("X.10", "X.01"), alphas = 3:4) |> 
...
The \pkg{CausalQueries} package provides function
...
Samples were drawn using NUTS(diag_e) at Thu Jul 10 19:07:06 2025.
...
Samples were drawn using NUTS(diag_e) at Thu Jul 10 19:07:25 2025.
For each parameter, n_eff is a crude measure of effective sample size,
and Rhat is the potential scale reduction factor on split chains (at 
convergence, Rhat=1).
\end{CodeOutput}
...
\begin{CodeInput}
R> queries <- query_model(models, query = 
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, colframe=quarto-callout-color-frame, arc=.35mm, breakable, toprule=.15mm, rightrule=.15mm, colback=white, bottomrule=.15mm, left=2mm, leftrule=.75mm]
...
\centering{
  ...
  \midrule
  $X1 \rightarrow Y$ & 6 & 868.83\\
  $X1 \rightarrow Y; X2 \rightarrow Y$ & 20 & 1926.42\\
  $X1\rightarrow Y;X2\rightarrow Y;X3\rightarrow Y$ & 262 & 39648.79\\
  \bottomrule
  \end{tabular}
...
\centering{
  ...
  \midrule
  $X1 \rightarrow Y$ & 10 & 942.23\\
  $X1 \rightarrow Y$ & 100 & 1279.47\\
  $X1 \rightarrow Y$ & 1000 & 2497.54\\
  $X1 \rightarrow Y$ & 10000 & 6162.23\\
  \bottomrule
  \end{tabular}
